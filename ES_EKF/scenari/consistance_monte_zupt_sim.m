...
% ═══════════════════════════════════════════════════════════════════════
traj = kinematicTrajectory('SampleRate', round(1/dt),'Position', [0 0 0], 'Velocity', [0 0 0], 'Orientation', q_init);
...
for trial = 1:N_TRIALS
    ...
    %   자세 perturbation: TRIAD 평균 정렬 오차 ≈ 1° 모사 (3축 IID Gaussian)
    ...
    R_bn_init = NavUtils.quat2dcm(q_true(:,1));
...
xlabel('Time [sec]'); ylabel('Norm 오차 [m]'); title('위치 오차 (Norm)');
...
legend('Location','northeast');
xlabel('Time [sec]'); ylabel('Norm 오차 [m/s]'); title('속도 오차 (Norm)');
...
legend('Location','northeast');
xlabel('Time [sec]'); ylabel('Norm 오차 [m/s²]'); title('가속도 바이어스 오차 (Norm)');
...
legend('Location','northeast');
xlabel('Time [sec]'); ylabel('Norm 오차 [rad/s]'); title('자이로 바이어스 오차 (Norm)');
...
xlabel('Time [sec]'); ylabel('NEES'); title('NEES 일관성 (≈6)');
