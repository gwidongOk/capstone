...
// -----------------------------------------------------------------------------
#define PYRO_1_PIN   18    // Pyro 1: 드로그/메인 낙하산 분리 (Apogee 시점 점화)
#define PYRO_2_PIN   8     // Pyro 2: 예비 채널 (현재 미사용)
...
// Core 1 (고속 실시간 제어 전용)
#define TASK_C1_PRIO_IMU     5     // IMU 416Hz 데이터-레디 인터럽트 처리 (최우선)
...
// 측정 잡음 분산 (R 행렬 대각)
#define EKF_VAR_BARO             4.920342e-2f                // 기압계 고도 분산 [m^2]
#define EKF_VAR_GPS_POS_H        (0.694f * 0.694f)           // GPS 수평위치 기본 분산 [m^2]
#define EKF_VAR_GPS_POS_V        (1.282927f * 1.282927f)     // GPS 수직위치 기본 분산 [m^2]
#define EKF_VAR_GPS_VEL_H        (0.2684742f * 0.2684742f)   // GPS 수평속도 기본 분산 [(m/s)^2]
#define EKF_VAR_GPS_VEL_V        (0.2684742f * 0.2684742f)   // GPS 수직속도 기본 분산 [(m/s)^2]
#define EKF_VAR_MAG              1.5e-5f                     // 자력계 단위벡터 분산
