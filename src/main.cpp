...
/* ---- Pyro pulse management (non-blocking) ---- */
static void pyro_service(void) {
  uint32_t now = millis();
  if (pyro1_active && (now - pyro1_start_ms > RECOVERY_PYRO_PULSE_MS)) {
    digitalWrite(PYRO_1_PIN, LOW);
    pyro1_active = false;
  }
  if (pyro2_active && (now - pyro2_start_ms > RECOVERY_PYRO_PULSE_MS)) {
    digitalWrite(PYRO_2_PIN, LOW);
    pyro2_active = false;
  }
}

/* ---- Drive pyro GPIO from recovery module flags ---- */
static void pyro_update_from_recovery(const recovery_state_t *rcv) {
  /* Drogue: PYRO_1 */
...
static void flight_task(void *arg) {
  ...
  for (;;) {
    ...
    /* ================================================================
     * 9. RECOVERY: drive pyro GPIOs from recovery module
     *    (skip in desk test and NAV_TEST modes)
     * ================================================================ */
    if (!desk_test_mode && !navTestActive) {
      pyro_update_from_recovery(&gnc_out.recovery);
    }
...
void loop() {
  ...
  /* ---- Pre-Flight Commands ---- */
    ...
    /* NAV_TEST: 항법 only 테스트 (서보/파이로 비활성) */
      ...
      int16_t gx, gy, gz, ax, ay, az;
      imu.readCalibratedIMU(gx, gy, gz, ax, ay, az);
      snprintf(buf, sizeof(buf), "IMU: g=[%d,%d,%d] a=[%d,%d,%d]\n", gx, gy, gz,
               ax, ay, az);
      sendResponse(buf);
      float alt;
      if (bmp.readAltitude(alt)) {
        snprintf(buf, sizeof(buf), "BMP: %.2f m\n", alt);
      } else {
        snprintf(buf, sizeof(buf), "BMP: FAIL\n");
      }
      sendResponse(buf);
