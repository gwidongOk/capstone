...
static volatile bool alignReady = false;
static volatile bool padAlignActive = false;
static volatile bool sensorTasksPaused = false;
...
bool runPadAlignment(const char *tag);
...
bool runStaticCalibration() {
  ...
  sendResponse("STATIC CALIB IMU+BARO...\n");
  if (xSemaphoreTake(spiMutex, pdMS_TO_TICKS(NAV_MUTEX_LONG_WAIT_MS)) == pdTRUE) {
    imuOk = imu.calibrate(CALIB_SAMPLES);
    bmpOk = bmp.calibrate(CALIB_SAMPLES);
    xSemaphoreGive(spiMutex);
  }

  sendResponse("STATIC CALIB GPS...\n");
  uint32_t gpsWaitStart = millis();
  while ((uint32_t)(millis() - gpsWaitStart) < 2000) {
    if (gps.update()) break;
    vTaskDelay(pdMS_TO_TICKS(GPS_POLL_MS));
  }
  gpsOk = gps.calibrate();
  if (gpsOk && xSemaphoreTake(navMutex, pdMS_TO_TICKS(NAV_MUTEX_LONG_WAIT_MS)) == pdTRUE) {
  ...
  if (!imuOk || !bmpOk || !gpsOk) {
    if (!imuOk) sendResponse("STATIC CALIB FAIL: IMU\n");
    if (!bmpOk) sendResponse("STATIC CALIB FAIL: BARO\n");
    if (!gpsOk) sendResponse("STATIC CALIB FAIL: GPS\n");
  ...
  digitalWrite(LED_PIN, LOW);
  beep(200);
  sendResponse("STATIC CALIB DONE\n");
  return true;
}
...
bool runPadAlignment(const char *tag) {
  ...
  char buf[80];
  snprintf(buf, sizeof(buf), "%s ZUPT %s in %.1fs (P_trace=%.3e)\n",
           tag, converged ? "CONVERGED" : "TIMEOUT",
...
//     * READY            : TRIAD + ZUPT 발사각 정렬
//     * START            : 정렬 중지 + 로깅 시작 + 발사 감지
//     * NAV_TEST         : 항법/로깅 테스트
//     * STOP             : 비행/테스트 종료
//     * PARSE            : 플래시 raw dump
// =============================================================================
...
void Flight_Task(void *pvParameters) {
  ...
  for (;;) {
    ...
    uint8_t main_parachute_count = 0;
    ...
    while (flightActive) {
      ...
      switch (flightPhase) {
        ...
        case FlightPhase::DESCENT:
          if (!main_deployed && alt < MAIN_PARACHUTE_ALT_M && vel_up < 0.0f) {
            main_parachute_count++;
            if (main_parachute_count >= MAIN_PARACHUTE_CONFIRM_COUNT) {
...
void IMU_Task(void *pvParameters) {
  ...
  uint32_t lastPadAlignMs = 0;
  for (;;) {
    ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
    if (sensorTasksPaused) continue;
    ...
    if (xSemaphoreTake(navMutex, pdMS_TO_TICKS(NAV_MUTEX_FAST_WAIT_MS)) == pdTRUE) {
      ...
      if (padAlignActive && !flightActive && nav.isEkfReady()) {
        State_imu imu_state = nav.getStateImu();
        State_nominal s = nav.getNominal();
        float ax = imu_state.ax - s.ba[0];
        float ay = imu_state.ay - s.ba[1];
        float az = imu_state.az - s.ba[2];
        float amag = sqrtf(ax*ax + ay*ay + az*az);
        if (amag > LAUNCH_ACCEL_MPS2) {
          padAlignActive = false;
          alignReady = false;
        } else if ((uint32_t)(nowMs - lastPadAlignMs) >= ZUPT_PERIOD_MS) {
          nav.ekfUpdateStaticAlignment();
          lastPadAlignMs = nowMs;
        }
      }
...
void MAG_Task(void *pvParameters) {
  ...
  for (;;) {
    if (sensorTasksPaused) {
      vTaskDelay(pdMS_TO_TICKS(MAG_POLL_MS));
      continue;
    }
    bool got = false;
    if (xSemaphoreTake(i2cMutex, portMAX_DELAY) == pdTRUE) {
