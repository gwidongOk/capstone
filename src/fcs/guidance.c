                            ...
                            float t)
{
    ...
    /* Ground velocity for guidance commands, airspeed for qbar */
    float V_ground = vec3_norm(nav->vel_ned);
    if (V_ground < 1.0f) V_ground = 1.0f;
    ...
    if (qbar < cfg->qbar_min_ctrl || t <= cfg->t_guide_on) {
        return out;
    }
    ...
    if (wind_mag > 0.01f) {
        ...
        if (t_go_wind > 10.0f) t_go_wind = 10.0f;
                                ...
                                const gnc_config_t *cfg)
{
    ...
    float K_guid = cfg->K_guid_far + (cfg->K_guid_near - cfg->K_guid_far) * progress;
    ...
    if (los_lead_n > 1.0f) {
        ...
        a_cmd = vec3_add(vec3_scale(a_lead, lead_w),
                         vec3_scale(a_pure, 1.0f - lead_w));
    } else {
