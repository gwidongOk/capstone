...
static inline vec3_t vec3_normalize(vec3_t v) {
    float n = vec3_norm(v);
    if (n < 1e-10f) return (vec3_t){0, 0, 0};
...
static inline quat_t quat_normalize(quat_t q) {
    ...
    if (n < 1e-10f) return (quat_t){1, 0, 0, 0};
    float inv_n = 1.0f / n;
...
static inline void quat_to_euler_zyx(quat_t q, float *phi, float *theta, float *psi) {
    float q0 = q.q0, q1 = q.q1, q2 = q.q2, q3 = q.q3;
...
static inline float interp1_linear(const float *V_bp, const float *table,
                                   int n, float V) {
    if (V <= V_bp[0]) return table[0];
    if (V >= V_bp[n-1]) return table[n-1];

    int i = 0;
    while (i < n-2 && V_bp[i+1] < V) i++;
